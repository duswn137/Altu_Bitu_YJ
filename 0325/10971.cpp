 ...
 */
void backtracking(int cnt, int cur_city, int cost) {    //순회 도시개수, 현재 도시, 비용
    if (cost >= ans) { //생각해보기 : 이 조건문이 없으면 어떻게 될까요?    return을 안함
        return;
    }
...
int main() {                                        //메인함수
    //입력
    cin >> n;
    visited.assign(n, false);                   //우선 전부 false로 초기화
    matrix.assign(n, vector<int>(n, 0));
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            cin >> matrix[i][j];            //비용 저장
        }
    }
