...
ll greedyOil(vector<pair<ll, ll>> &city, int n) {       //주유소 기름값 함수
    ll cur_cost = city[0].second, tot_cost = 0;         //현재 가격과 총 비용 선언 및 초기화

    for (int i = 0; i < n; i++) {
        if (city[i].second < cur_cost) { //이 도시의 기름값이 더 저렴하면 교체
            cur_cost = city[i].second;
        }
        tot_cost += (cur_cost * city[i].first); //이동 비용=기름값*도로 개수
    }
    return tot_cost;                                //리턴값
}
...
int main() {                                //메인함수
    ...
    //연산 & 출력
    cout << greedyOil(city, n);             //작성한 함수로 연산 후 답 출력
    return 0;
}